...
{
    /// <summary>
    /// FzkConsultationQueryView.xaml µÄ½»»¥Âß¼­
    /// </summary>
    [Export(typeof(IFzkConsultationQueryView))]
    public partial class FzkConsultationQueryView  : UserControl, IFzkConsultationQueryView
    {
        private readonly Lazy<FzkConsultationQueryViewModel> viewModel;
    
        public FzkConsultationQueryView()
        {
            InitializeComponent();
            viewModel = new Lazy<FzkConsultationQueryViewModel>(() => ViewHelper.GetViewModel<FzkConsultationQueryViewModel>(this));
        }
        ...
        private void gridConsultationLogbookList_PagingChanged(object sender, CustomControlLibrary.PagingChangedEventArgs args)
        {
            try
            {
                ...
                IQueryable<FzkConsultation> gridTables = viewModel.Value.FzkConsultations;

                gridConsultationLogbookList.Total = gridTables.Count();
                gridConsultationLogbookList.ItemsSource = gridTables.OrderByDescending(p => p.Id).Skip((args.PageIndex - 1) * args.PageSize).Take(args.PageSize);
            }

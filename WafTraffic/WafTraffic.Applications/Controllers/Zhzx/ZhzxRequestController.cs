...
{
    [Export]
    internal class ZhzxRequestController : Controller
    {
        ...
        private ZhzxRequestUpdateViewModel mRequestUpdateViewModel;
        private ZhzxRequestQueryViewModel mRequestQueryViewModel;
        ...
        public ZhzxRequestController(CompositionContainer container, 
            ...
            IShellService shellService, IEntityService entityService)
        {
            try
            {
                ...
                mRequestUpdateViewModel = container.GetExportedValue<ZhzxRequestUpdateViewModel>();
                mRequestQueryViewModel = container.GetExportedValue<ZhzxRequestQueryViewModel>();
        ...
        public void Initialize()
        {
            try
            {
                ...
                //下拉菜单中只显示邵传全
                mRequestUpdateViewModel.LeaderList = AuthService.Instance.SubLeaderGroup.Where(entry => entry.RealName == "邵传全").ToList();
        ...
        public bool AddOper()
        {
            ...
            try
            {
                if (!AuthService.Instance.ZhzxRequestCanAdd)
                {
                ...
                mRequestUpdateViewModel.EquipmentRequestEntity = new ZhzxEquipmentRequest();
                mRequestUpdateViewModel.SubLeaderSignImg = null;
                mRequestUpdateViewModel.DdzSignImg = null;

                BaseOrganizeEntity org = mRequestUpdateViewModel.DepartmentList.Find(
                    entity => (entity.Id == CurrentLoginService.Instance.CurrentUserInfo.DepartmentId));

                if (org != null)
                {
                    mRequestUpdateViewModel.EquipmentRequestEntity.RequestDept = (int)org.Id;
                    mRequestUpdateViewModel.EquipmentRequestEntity.RequestDeptName = org.FullName;   
                }

                mRequestUpdateViewModel.EquipmentRequestEntity.Applicant = CurrentLoginService.Instance.CurrentUserInfo.RealName;
        ...
        public bool CancelOper()
        {
            ...
            try
            {
                ...
                {
                    if (mRequestUpdateViewModel.EquipmentRequestEntity.Status == YcConstants.INT_ZHZX_REQSTAT_SUB_LEADER_APPROVE)
                    {
                        mRequestUpdateViewModel.SubLeaderSignImg = null;

                    }
                    else if (mRequestUpdateViewModel.EquipmentRequestEntity.Status == YcConstants.INT_ZHZX_REQSTAT_DDZ_APPROVE)
                    {
                        mRequestUpdateViewModel.DdzSignImg = null;
                    }
                }
        ...
        public bool QueryOper()
        {
            ...
            try
            {
                ...
                mRequestQueryViewModel.Requests = mEntityService.QueryZhzxEquipmentRequests(selectedStatus, startDate, endDate.AddDays(1));
            ...
            try
            {
                ...
                if (dlgResult == DialogResult.Yes)
                {
                    if (mRequestUpdateViewModel.EquipmentRequestEntity.Id > 0)
                    {
                        ...
                        {
                            if (mRequestUpdateViewModel.EquipmentRequestEntity.Status == YcConstants.INT_ZHZX_REQSTAT_ZHZX_EXECUTE)
                            {
                            ...
                            else if (mRequestUpdateViewModel.EquipmentRequestEntity.Status == YcConstants.INT_ZHZX_REQSTAT_SUB_LEADER_APPROVE)
                            {
        ...
        public bool SubmitOper()
        {
            ...
            try
            {
                ...
                if (mRequestUpdateViewModel.EquipmentRequestEntity.Id > 0)
                {
                    ...
                    if (nextStatus > YcConstants.INT_ZHZX_REQSTAT_NULL)
                    {
                ...
                else
                {
                    ...
                    mRequestUpdateViewModel.EquipmentRequestEntity.Status = YcConstants.INT_ZHZX_REQSTAT_SUB_LEADER_APPROVE;
                    mRequestUpdateViewModel.EquipmentRequestEntity.StatusDesc = YcConstants.STR_ZHZX_REQSTAT_SUB_LEADER_APPROVE;
                    ...
                    mEntityService.Entities.ZhzxEquipmentRequests.AddObject(mRequestUpdateViewModel.EquipmentRequestEntity);
        ...
        private bool ValueCheck()
        {
            ...
            if ((mRequestUpdateViewModel.EquipmentRequestEntity.RequestType == 0) ||
                ValidateUtil.IsBlank(mRequestUpdateViewModel.EquipmentRequestEntity.RequestDeptName) ||
                ValidateUtil.IsBlank(mRequestUpdateViewModel.EquipmentRequestEntity.Applicant) ||
                (mRequestUpdateViewModel.EquipmentRequestEntity.SubLeaderId == 0))
            {
            ...
            switch (curStatus)
            {
                case YcConstants.INT_ZHZX_REQSTAT_DDZ_APPROVE:
                    ...
                    {
                        DialogResult ret = MessageBox.Show("大队长未签名，无法提交", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Error);

                        return false;

                    }
                    break;
                case YcConstants.INT_ZHZX_REQSTAT_SUB_LEADER_APPROVE:
                    ...
                    {
                        DialogResult ret = MessageBox.Show("分管领导未签名，无法提交", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Error);

                        return false;  

                    }
                    break;
                case YcConstants.INT_ZHZX_REQSTAT_ZHZX_EXECUTE:
                    break;
                /*
        ...
        private void GetNextStatus(int curStatus, out int nextStatus, out string nextStatusDesc)
        {
            switch (curStatus)
            {
                case YcConstants.INT_ZHZX_REQSTAT_NULL:
                    nextStatus = YcConstants.INT_ZHZX_REQSTAT_SUB_LEADER_APPROVE;
                    nextStatusDesc = YcConstants.STR_ZHZX_REQSTAT_SUB_LEADER_APPROVE;
                    break;
                case YcConstants.INT_ZHZX_REQSTAT_SUB_LEADER_APPROVE:
                    nextStatus = YcConstants.INT_ZHZX_REQSTAT_DDZ_APPROVE;
                    nextStatusDesc = YcConstants.STR_ZHZX_REQSTAT_DDZ_APPROVE;
                    break;
                case YcConstants.INT_ZHZX_REQSTAT_DDZ_APPROVE:
                    nextStatus = YcConstants.INT_ZHZX_REQSTAT_BGS_EXECUTE;
                    nextStatusDesc = YcConstants.STR_ZHZX_REQSTAT_BGS_EXECUTE;
                    break;
                case YcConstants.INT_ZHZX_REQSTAT_BGS_EXECUTE:
                    nextStatus = YcConstants.INT_ZHZX_REQSTAT_ZHZX_EXECUTE;
                    nextStatusDesc = YcConstants.STR_ZHZX_REQSTAT_ZHZX_EXECUTE;
                    break;
                case YcConstants.INT_ZHZX_REQSTAT_ZHZX_EXECUTE:
                    nextStatus = YcConstants.INT_ZHZX_REQSTAT_COMPLETED;
                    nextStatusDesc = YcConstants.STR_ZHZX_REQSTAT_COMPLETED;
                    break;
                default:
                    nextStatus = YcConstants.INT_ZHZX_REQSTAT_NULL;
                    nextStatusDesc = YcConstants.STR_ZHZX_REQSTAT_NULL;
                    break;
            }
        }
        ...
        private void GetRejectStatus(int curStatus, out int rejectStatus, out string rejectStatusDesc)
        {
            ...
            {
                //case YcConstants.INT_ZHZX_REQSTAT_DDZ_APPROVE:
                //    rejectStatus = YcConstants.INT_ZHZX_REQSTAT_SUB_LEADER_APPROVE;
                //    rejectStatusDesc = YcConstants.STR_ZHZX_REQSTAT_SUB_LEADER_APPROVE;
                //    break;
                //case YcConstants.INT_ZHZX_REQSTAT_ZHZX_EXECUTE:
                //    rejectStatus = YcConstants.INT_ZHZX_REQSTAT_DDZ_APPROVE;
                //    rejectStatusDesc = YcConstants.STR_ZHZX_REQSTAT_DDZ_APPROVE;
                //    break;
                //case YcConstants.INT_ZHZX_REQSTAT_SUB_LEADER_APPROVE:
                default:
                    rejectStatus = YcConstants.INT_ZHZX_REQSTAT_NULL;
                    rejectStatusDesc = "已退回";
                    break;
            }
        ...
        private string GetDeptName(int deptId)
        {
            ...
            {
                foreach (BaseOrganizeEntity dept in mDepartmentList)
                {
                    if (dept.Id == deptId)
                    {
                        name = dept.FullName;
        ...
        {
            if (mRequestQueryViewModel.SelectedRequest != null)
            {
                DateTime deadLine = (DateTime)mRequestQueryViewModel.SelectedRequest.Deadline;
                if (deadLine != null)
                {

...
{
    /// <summary>
    /// ZgxcAskForLeaveQueryView.xaml µÄ½»»¥Âß¼­
    /// </summary>
    [Export(typeof(IZgxcAskForLeaveQueryView))]
    public partial class ZgxcAskForLeaveQueryView : UserControl, IZgxcAskForLeaveQueryView
    {
        private readonly Lazy<ZgxcAskForLeaveQueryViewModel> viewModel;
    
        public ZgxcAskForLeaveQueryView()
        {
            InitializeComponent();
            viewModel = new Lazy<ZgxcAskForLeaveQueryViewModel>(() => ViewHelper.GetViewModel<ZgxcAskForLeaveQueryViewModel>(this));
        }
        ...
        private void gridAskForLeaveList_PagingChanged(object sender, CustomControlLibrary.PagingChangedEventArgs args)
        {
            try
            {
                ...
                IQueryable<ZgxcAskForLeave> gridTables = viewModel.Value.ZgxcAskForLeaves;

                gridAskForLeaveList.Total = gridTables.Count();
                gridAskForLeaveList.ItemsSource = gridTables.OrderBy(p => p.Id).Skip((args.PageIndex - 1) * args.PageSize).Take(args.PageSize);
            }

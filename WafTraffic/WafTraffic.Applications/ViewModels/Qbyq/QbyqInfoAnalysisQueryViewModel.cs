...
{
    [Export]
    public class QbyqInfoAnalysisQueryViewModel : ViewModel<IQbyqInfoAnalysisQueryView>
    {
        #region Data

        private QbyqInfoAnalysisCase mSelectedCase;
        private IQueryable<QbyqInfoAnalysisCase> mPunishCases;
        ...
        public QbyqInfoAnalysisQueryViewModel(IQbyqInfoAnalysisQueryView view, IEntityService entityservice)
            : base(view)
        {
            if (entityservice.EnumQbyqInfoAnalysisCases != null)
            {
                this.mPunishCases = entityservice.EnumQbyqInfoAnalysisCases;
            }
            else
            {
                this.mPunishCases = new List<QbyqInfoAnalysisCase>().AsQueryable(); //以防没有数据时出现异常
            }
            ...
            if (CurrentLoginService.Instance.IsAuthorized(YcConstants.STR_AUTH_QBYQ_DELETE))
            {
                mDeletePermissionVisibility = Visibility.Visible;
            }
            if (CurrentLoginService.Instance.IsAuthorized(YcConstants.STR_AUTH_QBYQ_MODIFY))
            {
                mModifyPermissionVisibility = Visibility.Visible;
            }
            if (CurrentLoginService.Instance.IsAuthorized(YcConstants.STR_AUTH_QBYQ_BROWSE))
            {
                mBrowsePermissionVisibility = Visibility.Visible;
            }
            if (CurrentLoginService.Instance.IsAuthorized(YcConstants.STR_AUTH_QBYQ_ADD))
            {
        ...
        public IQueryable<QbyqInfoAnalysisCase> PunishCases
        {
            get
            {
                return mPunishCases;
        ...
        public QbyqInfoAnalysisCase SelectedCase
        {

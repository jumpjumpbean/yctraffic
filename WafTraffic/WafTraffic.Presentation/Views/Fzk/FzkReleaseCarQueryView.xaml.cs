...
{
    /// <summary>
    /// FzkReleaseCarQueryView.xaml µÄ½»»¥Âß¼­
    /// </summary>
    [Export(typeof(IFzkReleaseCarQueryView))]
    public partial class FzkReleaseCarQueryView : UserControl, IFzkReleaseCarQueryView
    {
        private readonly Lazy<FzkReleaseCarQueryViewModel> viewModel;
    
        public FzkReleaseCarQueryView()
        {
            InitializeComponent();
            viewModel = new Lazy<FzkReleaseCarQueryViewModel>(() => ViewHelper.GetViewModel<FzkReleaseCarQueryViewModel>(this));
        }
        ...
        private void gridAskForLeaveList_PagingChanged(object sender, CustomControlLibrary.PagingChangedEventArgs args)
        {
            try
            {
                ...
                IQueryable<FzkReleaseCar> gridTables = viewModel.Value.FzkReleaseCars;

                gridAskForLeaveList.Total = gridTables.Count();
                gridAskForLeaveList.ItemsSource = gridTables.OrderByDescending(p => p.Id).Skip((args.PageIndex - 1) * args.PageSize).Take(args.PageSize);
            }

...
{
    [Export]
    public class ZxkOrderQueryViewModel : ViewModel<IZxkOrderQueryView>
    {
        #region Data

        private ZxkOrderCase mSelectedCase;
        private IQueryable<ZxkOrderCase> mPunishCases;
        ...
        public ZxkOrderQueryViewModel(IZxkOrderQueryView view, IEntityService entityservice)
            : base(view)
        {
            if (entityservice.EnumZxkOrderCases != null)
            {
                this.mPunishCases = entityservice.EnumZxkOrderCases;
            }
            else
            {
                this.mPunishCases = new List<ZxkOrderCase>().AsQueryable(); //以防没有数据时出现异常
            }
            ...
            if (CurrentLoginService.Instance.IsAuthorized(YcConstants.STR_AUTH_ZXK_DELETE))
            {
                mDeletePermissionVisibility = Visibility.Visible;
            }
            if (CurrentLoginService.Instance.IsAuthorized(YcConstants.STR_AUTH_ZXK_MODIFY))
            {
                mModifyPermissionVisibility = Visibility.Visible;
            }
            if (CurrentLoginService.Instance.IsAuthorized(YcConstants.STR_AUTH_ZXK_BROWSE))
            {
                mBrowsePermissionVisibility = Visibility.Visible;
            }
            if (CurrentLoginService.Instance.IsAuthorized(YcConstants.STR_AUTH_ZXK_ADD))
            {
        ...
        public IQueryable<ZxkOrderCase> PunishCases
        {
            get
            {
                return mPunishCases;
        ...
        public ZxkOrderCase SelectedCase
        {
